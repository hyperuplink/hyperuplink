...
_None_, and the target you pick carries confirmation tokens and reply
...
configured with `InsecureSkipVerify`. Where it says true, everyone able to sit
between the board and the XMPP server can present a certificate of their own and
...
still starts and still serves, and it simply fails to deliver until the server
comes back.
