...
Forums are where topics live, and each of them belongs to exactly one
[category]({{ manual "admin/board/categories" }}), which is what decides who may
read and write in it.

![The forums page](forums.webp)

The page groups the forums under their categories, and each row carries the
...
arrows stop at the ends of the category the forum sits in, since ordering
...
[category]({{ manual "admin/board/categories" }}) it sits in. The delete is a
...
answers with a 404 on the URLs it used to have.
