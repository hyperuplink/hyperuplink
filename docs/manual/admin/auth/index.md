...
- _Jabber identifier (JID) only_: makes the board XMPP-native, and confirmation
  tokens as well as notifications then travel over XMPP.
...
Pick the one you can actually deliver to. An address type the board has no
...
to confirm that it has been verified, and it declines the sign-in otherwise,
which is worth knowing when a provider you configured refuses to let anyone in
and you're looking for the reason.
