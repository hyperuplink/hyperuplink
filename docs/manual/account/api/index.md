...
The board speaks JSON as well as HTML, over an interface that listens on a port
...
about. _Delete_ revokes a key immediately, and a script still holding it gets a
401 on its next request.

## Using a key

Every request includes the key, either as a bearer token or in an `X-API-Key`
header, and a request without one returns 401 whatever it asked for, because the
...
That call returns the account the key belongs to, which makes it the first
...
The board returns the location of the new topic. A key acts as you, it reads
what you may read and writes where you may write and nothing beyond that.
Handing a key to a script, however, gives it your account, and it needs the same
thought as handing over your password.
