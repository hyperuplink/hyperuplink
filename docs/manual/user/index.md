...
Every username on the board links to that user's profile, which lives at `/~`
plus the username, so `/~vera` is vera's. Your own is reachable from your name
at the right of the menu bar.

![A user profile](profile.webp)

The panel on the left carries the profile picture, the role, the groups the user
