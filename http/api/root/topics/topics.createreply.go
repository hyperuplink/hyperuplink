...
// @Summary		Reply to a topic by identifier
// @Description	Everyone subscribed to the topic is notified once the reply has been
// @Description	stored.
// @Tags			board
// @Accept			json
// @Produce		json
// @Param			id		path		string					true	"The topic identifier"
// @Param			request	body		topics.ReplyCreateBody	true	"The reply to store"
...
// @Security		BearerAuth
// @Security		APIKeyAuth
// @Router			/topics/{id}/replies [post]
func (r *Route) CreateReply(c fiber.Ctx) (err error) {
	req := request.New(r, c)
	...
	if _, perr := uuid.Parse(c.Params("id")); perr != nil {
		return req.RespondError(errs.ErrFormInvalid)
	}

	in := new(ReplyCreateBody)
