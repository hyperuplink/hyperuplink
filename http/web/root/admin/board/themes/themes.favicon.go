...
func (r *Route) FaviconUpload(c fiber.Ctx) (err error) {
	...
	if frm.CustomFavicon == nil || frm.CustomFavicon.Filename == "" {
		req.Flash.SetError(errors.New("favicon_upload_failed"))
		return req.RedirectToRoute(myRoute)
	}
	...
	if theme.CustomFavicon != "" {
		req.Flash.SetError(errors.New("favicon_already_set"))
		return req.RedirectToRoute(myRoute)
	}
	...
	if !strings.HasPrefix(mtype.String(), "image/") {
		req.Flash.SetError(errors.New("favicon_format_not_allowed"))
		return req.RedirectToRoute(myRoute)
	}
