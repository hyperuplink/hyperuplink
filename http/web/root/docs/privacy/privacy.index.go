...
func (r *Route) Index(c fiber.Ctx) (err error) {
	...
	if !settingGeneral.JSONValue.EnablePrivacyPolicy {
		return c.SendStatus(fiber.StatusNotFound)
	}

	var html string
	html, err = r.Runtime.Markdown.Convert(settingGeneral.JSONValue.PrivacyPolicy)
	if ret, rerr := req.RespondOnError(err); ret == true {
		return rerr
	}
